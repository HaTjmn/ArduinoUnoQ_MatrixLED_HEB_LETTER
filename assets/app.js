...
async function importH(file) {
  ...
  try {
    ...
    if (data && data.ok) {
      console.debug('[ui] importH imported frames:', data.count);
      await refreshFrames();
      if (data.frame) {
    ...
    // Error is already shown by fetchWithHandling
    console.error('[ui] importH failed', err);
  } finally {
...
if (importBtn && importFileInput) {
  ...
  importFileInput.addEventListener('change', () => {
    ...
    if (file) importH(file);
  });
} else {
  console.warn('[ui] import button or file input not found');
}
...
const uploadAlphabetBtn = document.getElementById('upload-alphabet');
const alphabetFileInput = document.getElementById('alphabet-file-input');
if (uploadAlphabetBtn && alphabetFileInput) {
  uploadAlphabetBtn.addEventListener('click', () => alphabetFileInput.click());
  alphabetFileInput.addEventListener('change', async () => {
    const file = alphabetFileInput.files && alphabetFileInput.files[0];
    alphabetFileInput.value = '';
    if (!file) return;
    uploadAlphabetBtn.disabled = true;
    try {
      const content = await file.text();
      const data = await fetchWithHandling(
        '/upload_alphabet',
      ...
      if (data && data.ok) {
        const missingMsg = data.missing && data.missing.length ? ` Missing: ${data.missing.join(' ')}` : '';
        showVectorText(
          `Alphabet uploaded: ${data.matched}/27 letters matched (${data.unmatched} frame(s) unmatched).${missingMsg} Restart the app to rebuild and flash the sketch.`
        );
      } else if (data && data.error) {
        showError(data.error);
      }
    } catch (err) {
      console.error('[ui] uploadAlphabet failed', err);
    } finally {
      uploadAlphabetBtn.disabled = false;
    }
  });
} else {
  console.warn('[ui] upload alphabet button or file input not found');
}
...
if (writeSentenceBtn) {
  writeSentenceBtn.addEventListener('click', async () => {
    ...
    if (!text) {
      showError('Enter a sentence to write');
    ...
    try {
      ...
      console.error('[ui] writeSentence failed', err);
    }
  });
}
